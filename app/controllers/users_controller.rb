class UsersController < ApplicationController
  ...
  def create
    ...
    when "email_account"
      ...
      if @user.save
        otp = rand(1000..9999)
        email_otp = EmailOtp.new(otp_expiry: Time.now + 1.minutes,otp_number: otp,user_id: @user.id)
        if email_otp.save
    ...
    when "social_account"
      ...
      @user = SocialAccount.create(user_params)
  ...
  def forgot_password
    ...
    if user && user.mobile_verified?
      otp = rand(1000..9999)
      sms_otp = SmsOtp.new(otp_expiry: Time.now + 1.minutes,otp_number: otp,user_id: user.id)
      if sms_otp.save
     
        render json: { success: true, messages: 'OTP sent successfully to phone number.', otp: otp, user: user }, status: :ok
      else
        render json: { errors: sms_otp.errors.full_messages, success: false }, status: :unprocessable_entity
      end
    else
      render json: { errors: "Phone number not verified or user not found.", success: false }, status: :unprocessable_entity
    end
    else
      render json: { errors: "Email or phone number is missing.", success: false }, status: :unprocessable_entity
    end
  end
